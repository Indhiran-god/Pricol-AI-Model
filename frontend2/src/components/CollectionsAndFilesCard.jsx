...
const CollectionsAndFilesCard = ({ theme, user, collections, setCollections, onCollectionChange }) => {
  ...
  const refreshCollections = async () => {
    try {
      setLoading(true);
      const fetchResp = await fetch(`${API_BASE}/api/documents/collections?user_id=${userId}`);
      const fetchData = await fetchResp.json();
      if (fetchResp.ok) {
        setCollections(fetchData.collections || []);
        const initialExpanded = {};
        fetchData.collections.forEach((coll) => {
          initialExpanded[coll.id] = false;
        });
        setExpandedCollections(initialExpanded);
        onCollectionChange(fetchData.collections || []);
        toast.success("Collections refreshed successfully!");
      } else {
        toast.error(fetchData.error || "Failed to fetch collections");
      }
    } catch (err) {
      toast.error(`Error: ${err.message}`);
    } finally {
...
export default CollectionsAndFilesCard;